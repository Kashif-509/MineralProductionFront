...
const MineralRevenueTable = () => {
  ...
  }, [selectedYear, selectedMonth]); // Include selectedMonth in dependency array
  ...
  const fetchData = async (year) => {
    ...
    try {
      ...
      setData(response.data.result || []); // Ensure setData receives an array, even if empty
      setLoading(false);
      setError(null); // Clear error on successful data fetch
    } catch (error) {
      console.error("Error fetching data:", error);
      setData([]); // Set data to empty array on error
      setError("Error fetching data"); // Set appropriate error message
      setLoading(false);
    }
  };
